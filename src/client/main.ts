...
import { CELL_HEIGHT, CELL_WIDTH } from '../core/cell';
import { Position2D } from '../core/position2d';
import { Orientation } from '../core/orientation';
import { HexPosition, turn_left, turn_right } from '../core/hex_position';
...
export const SCALE = canvasWidth / 900;
...
function createBackground() {
    ...
    for (let i = 0; i <= window.innerWidth / 32; ++i) {
        background[i] = [];
        for (let j = 0; j <= window.innerHeight / 32; ++j) {
...
function update() {
    ...
    if (hexpos.within_given_box(gridWidth, gridHeight)) {
...
function position2DToPixels(x, y): Position2D {
    const offset = 50;
    let pix_coords = new Position2D(
        SCALE * (CELL_HEIGHT * x) + 50 * SCALE + offset,
        SCALE * (CELL_HEIGHT * y) + 50 * SCALE + offset
    );

    return pix_coords;
}

function pixelstoHexPosition(px, py): HexPosition {
    const offset = 50;
    let position2D = new Position2D(
        (px - 50 * SCALE - offset) / SCALE / CELL_HEIGHT,
        (py - 50 * SCALE - offset) / SCALE / CELL_HEIGHT
    );

    let col = Math.round(position2D.x / 0.866);
    let x = col;
    let odd_offset = 0;
    if (x % 2 == 1) {
        odd_offset = - 0.5;
    ...
    let row = Math.round(position2D.y + odd_offset);
...
function createPlayerShip(position: HexPosition, orientation: Orientation) {
