...
export function turn_right(orientation, factor) {
    if (!factor) factor = 1;
...
export function turn_left(orientation, factor) {
    if (!factor) factor = 1;
...
export class HexPosition {
    ...
    move_towards(orientation, distance): HexPosition {
    ...
    move_towards_unrestricted(orientation, distance): HexPosition {
        let hexPosition = HexPosition.unitary_vector(orientation);
        hexPosition = hexPosition.product(distance);
    ...
    get_neighbours() {
        let neighbours = [];
        [Orientation.U, Orientation.UR, Orientation.DR, Orientation.D, Orientation.DL, Orientation.UL].forEach(function (orientation) {
...
function test_hexposition() {
    let tests: [HexPosition, HexPosition, number, Orientation][] = [
        [new HexPosition(3, 0, -3), new HexPosition(1, 0, -1), 2, Orientation.DL],
